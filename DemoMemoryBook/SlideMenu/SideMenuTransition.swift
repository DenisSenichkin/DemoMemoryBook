...
open class SideMenuTransition: UIPercentDrivenInteractiveTransition {
    ...
    fileprivate class var presentingViewControllerForMenu: UIViewController? {
        get {
            return SideMenuManager.menuLeftNavigationController?.presentingViewController
        }
    }
    ...
    internal class func handlePresentMenuPan(_ pan: UIPanGestureRecognizer) {
        ...
        case .began, .changed:
            ...
            } else if distance < 0 && SideMenuTransition.presentDirection == .left {
                SideMenuTransition.presentDirection = .right
                switchMenus = true
                singleton.cancel()
            } else {
    ...
    internal class func hideMenuStart() {
        guard let mainViewController = presentingViewControllerForMenu, let menuView = SideMenuManager.menuLeftNavigationController?.view else {
                return
        }
